...
Describe "TerminalExpressions" {
	context "Initialize" {
		...
		It "CreateAllignedTable" {
			#$CustomizedMessage = CreateAllignedTable @("Path:","Looking Good") @("Short Path:","Looking Good?") @("Short Path:","Looking Good?")
			#write-host $CustomizedMessage
		}
		
		It "CreateAllignedMessage" {
			if($False)
			{
				$CustomizedMessage = CreateAllignedMessage @("Path:","Looking Good") @("Short Path:","Looking Good?")
				write-host $CustomizedMessage
			}
		}
}
